...
import { queryOne, upsertGameStats } from '@/lib/db';
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (userId) {
            ...
            try {
                ...
                const currentStats = await queryOne<{
                    ...
                    FROM gamestats 
                    WHERE user_id = ${userId} AND mode = ${'trivia'}`;
                ...
                await upsertGameStats(userId, 'trivia', newStreak, newBestStreak);
        ...
        console.error('Trivia guess error:', error);
        return NextResponse.json(
